...
const DocumentsForProcess = ({
  ...
  const removeBrbDocument = (index: number) => {
    setBrbDocuments((prevDocuments) =>
    ...
    <div className="bodyForDocProcess">
      ...
      {addDocumentModal && (
        <NewDocument
                  ...
                  loading={loading}
                  selectedFlow={selectedFlow} doctype={doctype}/>
      )}
      ...
      <div className="options">
        <div className="searchDiv">
          ...
          />{" "}
          <button onClick={searchDossier} disabled={!enableSearch}>
            Search
          </button>
        </div>
        <div className="docButtons">
          <button disabled={!enableAdd} onClick={() => {openAddDocumentModal(); setDoctype('document');}}>
            Add Document
          </button>
          <button disabled={!enableAdd} onClick={() => {openAddDocumentModal(); setDoctype('selfie');}}>
            Add Selfie
          </button>
          <button
            disabled={!enableRemove}
            onClick={() => selectedFlow.id != 15? removeDocument(selectedRow): removeBrbDocument(selectedRow)}
          >
            Remove Document
          </button>
